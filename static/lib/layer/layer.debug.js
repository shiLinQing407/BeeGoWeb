        ...
        callback([
            ...
            '<div class="' + doms[0] + (' layui-layer-' + ready.type[config.type]) + (((config.type == 0 || config.type == 2) && !config.shade) ? ' layui-layer-border' : '') + ' ' + (config.skin || '') + '" id="' + doms[0] + times + '" type="' + ready.type[config.type] + '" times="' + times + '" showtime="' + config.time + '" conType="' + (conType ? 'object' : 'string') + '" style="z-index: ' + zIndex + '; width:' + config.area[0] + ';height:' + config.area[1] + (config.fixed ? '' : ';position:absolute;') + '">'
                    ...
                    + (config.btn ? function () {
                        ...
                        var div = '<div class="' + doms[6] + ' layui-layer-btn-' + (config.btnAlign || '') + '';
                        if (config.type == 2) {
                            div += ' js_layui-layer-btn_wrap';
                        }
                        div += '">' + button + '</div>';
                        return div;
                    }() : '')
...
//拖拽层
    ...
    Class.pt.callback = function () {
        ...
        layero.find('.' + doms[6]).children('a').on('click', function () {
            ...
            if (index === 0) {//lcj修改
                if (config.yes) {
                    config.yes.call(layer, that.index, layero);
                } else if (config['btn1']) {
                    config['btn1'].call(layer, that.index, layero);
                } else {
                    layer.close(that.index);
                }
            } else {
                var close = config['btn' + (index + 1)] && config['btn' + (index + 1)].call(layer, that.index, layero);
